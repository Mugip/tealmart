    ...
    <div className="min-h-screen bg-gray-50 overflow-x-hidden pb-24 lg:pb-0">
        ...
        <div className="flex items-center gap-3 max-w-lg mx-auto">
          ...
          <button
            ...
            className={`p-3 rounded-xl border-2 transition-all ${
              wishlisted ? 'border-red-500 bg-red-50 text-red-500' : 'border-gray-200 text-gray-400'
            }`}
          >
            <Heart size={20} className={wishlisted ? 'fill-red-500' : ''} />
          </button>
      ...
      {isZoomed && (
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4" onClick={() => setIsZoomed(false)}>
          <button onClick={() => setIsZoomed(false)} className="absolute top-4 right-4 bg-white/10 hover:bg-white/20 p-3 rounded-full transition-all z-10">
          ...
          {allImages.length > 1 && (
            <>
              <button onClick={(e) => { e.stopPropagation(); prevImage() }} className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/20 p-4 rounded-full transition-all">
                <ChevronLeft size={32} className="text-white" />
              </button>
              <button onClick={(e) => { e.stopPropagation(); nextImage() }} className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/20 p-4 rounded-full transition-all">
      ...
      <style jsx global>{`
        ...
        .product-description table td, .product-description table th { padding: 0.5rem; border: 1px solid #e5e7eb; }
      `}</style>
    </div>
  )
}
