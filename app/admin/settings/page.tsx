...
export default function AdminSettingsPage() {
  const [activeTab, setActiveTab] = useState('general')                                                       const [saving, setSaving] = useState(false)           const [loading, setLoading] = useState(true)
  const [message, setMessage] = useState('')            const [showNewPassword, setShowNewPassword] = useState(false)
                                                        const [settings, setSettings] = useState({
    storeName: 'TealMart',
    storeEmail: 'support@tealmart.com',
    storePhone: '',                                       currency: 'USD',
    orderNotifications: true,                             lowStockAlerts: true,
    dailyReports: false,
    customerMessages: true,
    maintenanceMode: false,
    allowGuestCheckout: true,
    requireEmailVerification: false,                      autoApproveReviews: false,
  })
  ...
  // Load settings on mount                             useEffect(() => {
    ...
    setSaving(true)
    setMessage('')
    try {                                                   const res = await fetch('/api/admin/settings', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(settings),
      })
      const data = await res.json()                         if (!res.ok) {                                          setMessage(data.error || 'Failed to save settings')                                                       } else {
  ...
  return (
    <div className="p-4 lg:p-6 max-w-4xl mx-auto">          <div className="mb-6">
        <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Settings</h1>
        <p className="text-gray-600 text-sm mt-1">Manage your store preferences</p>                               </div>
                                                            {/* Tabs */}
      <div className="bg-white rounded-lg border border-gray-200 mb-6">
        <div className="overflow-x-auto">                       <div className="flex border-b border-gray-200">                                                               {tabs.map(tab => {
              const Icon = tab.icon                                 return (
                <button                                                 key={tab.id}                                          onClick={() => setActiveTab(tab.id)}                  className={`flex items-center gap-2 px-5 py-3.5 text-sm font-medium transition-colors whitespace-nowrap border-b-2 -mb-px ${
                    activeTab === tab.id
        ...
        <div className="p-6">
          ...
          {activeTab === 'general' && (                           <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="tel"                                            value={settings.storePhone}
                    onChange={e => setSettings(s => ({ ...s, storePhone: e.target.value }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-tiffany-500 focus:border-transparent"                               placeholder="+1 (555) 000-0000"
                  />
                </div>
                <div>
          ...
          {activeTab === 'security' && (                          <div className="space-y-4">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-sm text-amber-800">
                <Lock size={14} className="inline mr-2" />                                                                  Admin passwords are stored as bcrypt hashes in your environment variables. To change your password, generate a new hash and update <code className="bg-amber-100 px-1 rounded">ADMIN_PASSWORD_HASH</code> in your <code className="bg-amber-100 px-1 rounded">.env</code> file.
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="Disabled — use env variable"                                                                   disabled
                    />                                                    <button
                      type="button"
                      onClick={() => setShowNewPassword(!showNewPassword)}
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400"
                    >
                      {showNewPassword ? <EyeOff size={16} /> : <Eye size={16} />}                                              </button>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* Store Tab */}
          {activeTab === 'store' && (                             <div className="space-y-4">
              ...
              {[
                ...
                <label key={item.key} className={`flex items-start gap-3 cursor-pointer group p-3 rounded-lg ${item.danger && (settings as any)[item.key] ? 'bg-red-50 border border-red-200' : 'hover:bg-gray-50'}`}>
                  <input                                                  type="checkbox"
                    ...
                    className="w-4 h-4 mt-0.5 text-tiffany-600 rounded border-gray-300 focus:ring-tiffany-500"                                                                      />
                  <div>
                    <p className={`text-sm font-medium ${item.danger && (settings as any)[item.key] ? 'text-red-700' : 'text-gray-900'}`}>{item.label}</p>                            <p className="text-xs text-gray-500">{item.desc}</p>
                  </div>
                </label>
              ))}                                                 </div>
          )}                                                  </div>
      </div>                                          
      {/* Save */}                                          {message && (                                           <p className={`mb-4 text-sm font-medium ${message.startsWith('✓') ? 'text-green-600' : 'text-red-600'}`}>                                                           {message}
        </p>                                                )}                                                    <button
        onClick={handleSave}                                  disabled={saving}                                     className="inline-flex items-center gap-2 px-6 py-2.5 bg-tiffany-500 text-white rounded-lg hover:bg-tiffany-600 transition-colors font-medium text-sm disabled:opacity-50 disabled:cursor-not-allowed"                >
