...
export default function StaffPage() {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl mx-auto">
      {/* ✅ FIXED: Stack header properly on mobile screens */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            {AVAILABLE_PERMISSIONS.map(p => (
              <label key={p.id} className={`flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${permissions.includes(p.id) ? 'border-tiffany-500 bg-tiffany-50' : 'border-gray-100 hover:border-gray-200'}`}>
                <input type="checkbox" checked={permissions.includes(p.id)} onChange={() => togglePermission(p.id)} className="mt-1 w-4 h-4 text-tiffany-600 flex-shrink-0" />
                <div>
      ...
      {/* ✅ FIXED: Added overflow-x-auto to contain the table scroll on mobile */}
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase whitespace-nowrap">Staff Member</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase whitespace-nowrap">Granted Permissions</th>
            ...
            <tbody className="divide-y divide-gray-50">
              <tr className="bg-gray-50">
                ...
                <td className="px-6 py-4">
                  <span className="px-2.5 py-1 bg-gray-900 text-white text-[10px] font-bold rounded-full uppercase tracking-wider whitespace-nowrap">
                    Unrestricted Access
                  </span>
                </td>
                <td className="px-6 py-4 text-right text-gray-400 text-sm whitespace-nowrap">Cannot be deleted</td>
              </tr>
              {staffList.map((staff) => (
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-1.5 min-w-[200px]">
                      {staff.permissions.map((p: string) => (
                        <span key={p} className="px-2 py-1 bg-tiffany-100 text-tiffany-700 text-[10px] font-bold rounded-md uppercase tracking-wider whitespace-nowrap">
                          {p}
                        </span>
                      ))}
                    </div>
