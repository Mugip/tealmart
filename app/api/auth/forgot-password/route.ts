...
export async function POST(req: NextRequest) {
  try {
    ...
    // Always return success to prevent email enumeration                                                       const user = await prisma.user.findUnique({ where: { email } })

    if (user) {
      // Generate a secure token
      const rawToken = crypto.randomBytes(32).toString('hex')                                                     const hashedToken = crypto.createHash('sha256').update(rawToken).digest('hex')
      const expires = new Date(Date.now() + 60 * 60 * 1000) // 1 hour

      // Store token (using VerificationToken table from NextAuth schema)                                         await prisma.verificationToken.upsert({
