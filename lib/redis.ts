...
// Serverless-optimized Redis connection with Lazy Connect
export const redis =
  globalForRedis.redis ??
  (process.env.REDIS_URL
    ? new Redis(process.env.REDIS_URL, {
        lazyConnect: true,       // CRITICAL: Don't block the server boot process
...
export async function fetchWithCache<T>(
  ...
  // If Redis is not configured, just run the database query instantly
  if (!redis) {
    return fetcher()
  }
  ...
  try {
    ...
    // Fire-and-forget: set cache in the background
