...
import { getSecureImageUrl } from '@/lib/imageUrl' // ✅ IMPORTANT
  ...
  return (
    <div className="mt-12 bg-tiffany-50/50 rounded-3xl p-6 sm:p-8 border border-tiffany-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {upsells.map((product) => {
          const image = getSecureImageUrl(product.images?.[0]) // ✅ unified

          return (
            <div
              key={product.id}
              className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex flex-row md:flex-col items-center gap-4"
            >
              ...
              <div className="flex-1 text-left md:text-center min-w-0">
                <Link
                  ...
                  className="text-sm font-bold text-gray-900 line-clamp-1 hover:text-tiffany-600"
                >
                  {product.title}
                </Link>

                <p className="text-tiffany-600 font-bold mt-1">
