  ...
  return (
    ...
    >
      ...
      <div className="max-w-[1400px] mx-auto px-3 sm:px-4 lg:px-6 py-3 lg:py-4">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px_240px] gap-3 lg:gap-3 items-stretch lg:grid-rows-[560px]">
          ...
          >
            {/* Mobile top: fixed height so images never shift the layout */}
            <div className="flex gap-0" style={{ height: 210 }}>
              {/* Image pane — left half */}
              <div
                key={`mob-img-${currentIndex}`}
                className="relative flex-shrink-0 img-fade-in overflow-hidden"
                style={{ width: '42%' }}
              >
                <Image
              ...
              <div className="flex-1 flex flex-col p-3 min-w-0">
                <div className="space-y-1.5">
                  ...
                  <div key={`mob-t-${currentIndex}`} className="hero-slide-in" style={{ minHeight: '2.5rem', overflow: 'hidden' }}>
                    <h1
                      className="text-sm font-black text-white leading-snug line-clamp-2"
                  ...
                  <div style={{ minHeight: '3.5rem' }}>
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5" style={{ minHeight: '1rem' }}>
                      {current.compareAtPrice && current.compareAtPrice > current.price ? (
                        <>
            ...
            {/* Mobile CTAs: Moved outside fixed-height container to prevent overlap */}
            <div className="flex gap-2 px-3 py-2.5" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
              <Link
              ...
              {displayItems.map((product, idx) => (
                ...
                >
                  <div className="relative w-full h-full">
                    <Image src={getSecureImageUrl(product.images?.[0])} alt={product.title} fill className="object-cover" />
                  </div>
                </button>
              ))}
            </div>
          ...
          >
            ...
            <div className="relative z-10 flex-1 overflow-hidden flex flex-col gap-3">
              ...
              {/* Title — min-h reserves 2-line space so layout never shifts */}
              <div key={`dt-${currentIndex}`} className="hero-slide-in" style={{ minHeight: '4.5rem', overflow: 'hidden' }}>
                <h1
                  className="text-2xl xl:text-3xl font-black text-white leading-tight tracking-tight line-clamp-2"
                  style={{ fontFamily: "'DM Serif Display', Georgia, serif" }}
                >
              ...
              {/* Price — fixed height so CTA never shifts */}
              <div style={{ minHeight: '4.5rem' }}>
                <span className="text-4xl xl:text-5xl font-black text-white tracking-tight block leading-none">
                  {formatPrice(current.price)}
                </span>
                {/* Always render savings row — use visibility to hold space */}
                <div className="flex items-center gap-2 mt-1.5" style={{ minHeight: '1.25rem' }}>
                  {current.compareAtPrice && current.compareAtPrice > current.price ? (
                    <>
              ...
              {/* Countdown — always rendered, visibility toggled so space is always reserved */}
              <div style={{ visibility: (countdown.h > 0 || countdown.m > 0 || countdown.s > 0) ? 'visible' : 'hidden' }}>
                <div
            ...
            {/* CTAs — flex-shrink-0 pins them to the bottom regardless of content height */}
            ...
            {/* Trust micro-badges */}
            <div className="relative z-10 grid grid-cols-3 gap-2 flex-shrink-0">
              {[
          ...
          ══════════════════════════════ */}
          <div
            className="hidden lg:flex flex-col gap-2.5 rounded-2xl p-2.5 overflow-hidden"
            style={{
              background: '#0d1520',
              border: '1px solid rgba(255,255,255,0.055)',
              height: 560,
